...
// ../tx-sender/dist/index.js
var import_kit3 = require("@solana/kit");
var import_kit4 = require("@solana/kit");

// ../../node_modules/@solana-program/address-lookup-table/dist/src/index.mjs
var import_kit = require("@solana/kit");
function getAddressLookupTableDecoder() {
  return (0, import_kit.getStructDecoder)([
    ["discriminator", (0, import_kit.getU32Decoder)()],
    ["deactivationSlot", (0, import_kit.getU64Decoder)()],
    ["lastExtendedSlot", (0, import_kit.getU64Decoder)()],
    ["lastExtendedSlotStartIndex", (0, import_kit.getU8Decoder)()],
    [
      "authority",
      (0, import_kit.getOptionDecoder)((0, import_kit.getAddressDecoder)(), { noneValue: "zeroes" })
    ],
    ["padding", (0, import_kit.getU16Decoder)()],
    ["addresses", (0, import_kit.getArrayDecoder)((0, import_kit.getAddressDecoder)(), { size: "remainder" })]
  ]);
}
...
function getSetComputeUnitLimitInstructionDataEncoder() {
  return (0, import_kit2.transformEncoder)(
    (0, import_kit2.getStructEncoder)([
      ["discriminator", (0, import_kit2.getU8Encoder)()],
      ["units", (0, import_kit2.getU32Encoder)()]
    ]),
    (value) => ({
      ...value,
...
function getSetComputeUnitPriceInstructionDataEncoder() {
  return (0, import_kit2.transformEncoder)(
    (0, import_kit2.getStructEncoder)([
      ["discriminator", (0, import_kit2.getU8Encoder)()],
      ["microLamports", (0, import_kit2.getU64Encoder)()]
    ]),
    (value) => ({
      ...value,
...
function rpcFromUrl(url) {
  ...
  const transport = (0, import_kit3.createDefaultRpcTransport)({ url });
  const rpc = createRpc({ api, transport });
  return rpc;
}
function normalizeAddresses(addresses) {
  return addresses?.map((addr) => (0, import_kit3.address)(addr)) ?? [];
}
var globalConfig = {};
...
async function processJitoTipForTxMessage(message, signer, jito, chainId) {
  ...
  if (jitoTipLamports > 0) {
    return (0, import_kit6.prependTransactionMessageInstruction)(
...
async function recentJitoTip(priorityFeePercentile) {
  ...
  return (0, import_kit6.lamports)(BigInt(Math.floor(Number(data[key]) * 10 ** 9))).valueOf();
}
...
async function processComputeBudgetForTxMessage(message, computeUnits) {
  ...
  if (priorityFeeMicroLamports > 0) {
    message = (0, import_kit7.prependTransactionMessageInstruction)(
  ...
  ixs.forEach((ix) => {
    if (ix.accounts) {
      ix.accounts.forEach((acc) => {
        if ((0, import_kit7.isWritableRole)(acc.role)) writable.add(acc.address);
      });
    }
  });
...
async function getComputeUnitsForTxMessage(rpc, txMessage) {
  const estimator = (0, import_kit5.getComputeUnitEstimateForTransactionMessageFactory)({
    rpc
  });
...
async function buildTransaction(instructions, feePayer, lookupTableAddresses) {
  return buildTransactionMessage(
    instructions,
    !("address" in feePayer) ? (0, import_kit4.createNoopSigner)(feePayer) : feePayer,
...
async function buildTransactionMessage(instructions, signer, lookupTableAddresses) {
  ...
  return (0, import_kit4.signTransactionMessageWithSigners)(
...
async function buildAndSendTransaction(instructions, payer, lookupTableAddresses, commitment = "confirmed") {
  ...
  (0, import_kit8.assertTransactionIsFullySigned)(tx);
...
async function sendTransaction(transaction, commitment = "confirmed") {
  ...
  const encodedTransaction = (0, import_kit8.getBase64EncodedWireTransaction)(transaction);
...
function getTxHash(transaction) {
  ...
  const txHash = (0, import_kit8.getBase58Decoder)().decode(signature);
  return txHash;
}

// src/config.ts
var DEFAULT_ADDRESS = (0, import_kit9.address)("11111111111111111111111111111111");
var DEFAULT_WHIRLPOOLS_CONFIG_ADDRESSES = {
  solanaMainnet: (0, import_kit9.address)("2LecshUwdy9xi7meFgHtFJQNSKk4KdTrcpvaB56dP2NQ"),
  solanaDevnet: (0, import_kit9.address)("FcrweFY1G9HJAHG5inkGB6pKg1HZ6x9UC2WioAfWrGkR"),
  eclipseMainnet: (0, import_kit9.address)("FVG4oDbGv16hqTUbovjyGmtYikn6UBEnazz6RVDMEFwv"),
  eclipseTestnet: (0, import_kit9.address)("FPydDjRdZu9sT7HVd6ANhfjh85KLq21Pefr5YWWMRPFp")
};
var DEFAULT_WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS = (0, import_kit9.address)(
...
async function setWhirlpoolsConfig(config) {
  if ((0, import_kit9.isAddress)(config)) {
...
var DEFAULT_FUNDER = (0, import_kit9.createNoopSigner)(DEFAULT_ADDRESS);
var FUNDER = DEFAULT_FUNDER;
function setDefaultFunder(funder) {
  if (typeof funder === "string") {
    FUNDER = (0, import_kit9.createNoopSigner)(funder);
  } else {
    FUNDER = funder ?? (0, import_kit9.createNoopSigner)(DEFAULT_ADDRESS);
  }
}
...
async function setPayerFromBytes(pkBytes) {
  const kp = await (0, import_kit9.createKeyPairFromBytes)(pkBytes);
  const signer = await (0, import_kit9.createSignerFromKeyPair)(kp);
  _payer = signer;
  return signer;
}
function getPayer() {
...
var NATIVE_MINT = (0, import_kit10.address)(
...
async function prepareTokenAccountsInstructions(rpc, owner, spec) {
  ...
  if (hasNativeMint && NATIVE_MINT_WRAPPING_STRATEGY === "keypair") {
    const keypair = await (0, import_kit10.generateKeyPairSigner)();
    const space = (0, import_token2.getTokenSize)();
    let amount = await rpc.getMinimumBalanceForRentExemption(BigInt(space)).send();
    if (!Array.isArray(spec)) {
      amount = (0, import_kit10.lamports)(amount + BigInt(spec[NATIVE_MINT]));
    }
    createInstructions.push(
      (0, import_system2.getCreateAccountInstruction)({
        payer: owner,
        newAccount: keypair,
        lamports: amount,
        space,
  ...
  if (hasNativeMint && NATIVE_MINT_WRAPPING_STRATEGY === "seed") {
    ...
    const buffer = await crypto.subtle.digest(
      "SHA-256",
      Buffer.concat([
        Buffer.from((0, import_kit10.getAddressEncoder)().encode(owner.address)),
        Buffer.from(seed),
        Buffer.from((0, import_kit10.getAddressEncoder)().encode(import_token.TOKEN_PROGRAM_ADDRESS))
      ])
    );
    tokenAccountAddresses[NATIVE_MINT] = (0, import_kit10.getAddressDecoder)().decode(
      new Uint8Array(buffer)
    );
    createInstructions.push(
      (0, import_system2.getCreateAccountWithSeedInstruction)({
        payer: owner,
...
function orderMints(mint1, mint2) {
  const encoder = (0, import_kit10.getAddressEncoder)();
...
async function wouldExceedTransactionSize(currentInstructions, instructionsToAdd) {
  const noopSginer = (0, import_kit11.createNoopSigner)(
    (0, import_kit11.address)("11111111111111111111111111111111")
  );
  const tx = await buildTransaction(
    [...currentInstructions, ...instructionsToAdd],
    noopSginer
  );
  const encodedTransaction = (0, import_kit11.getBase64EncodedWireTransaction)(tx);
...
async function createConcentratedLiquidityPoolInstructions(rpc, tokenMintA, tokenMintB, tickSpacing, initialPrice = 1, funder = FUNDER) {
  ...
  const [
    ...
    (0, import_kit12.generateKeyPairSigner)(),
    (0, import_kit12.generateKeyPairSigner)()
  ]);
  instructions.push(
  ...
  return {
    instructions,
    poolAddress,
    initializationCost: (0, import_kit12.lamports)(nonRefundableRent)
  };
}
...
async function fetchPositionsForOwner(rpc, owner) {
  ...
  const encoder = (0, import_kit13.getBase64Encoder)();
  ...
  const [positions, positionBundles] = await Promise.all([
    (0, import_whirlpools_client4.fetchAllMaybePosition)(rpc, positionAddresses),
    (0, import_whirlpools_client4.fetchAllMaybePositionBundle)(rpc, positionBundleAddresses)
  ]);
  ...
  const bundledPositions = await (0, import_whirlpools_client4.fetchAllPosition)(
    rpc,
...
async function internalOpenPositionInstructions(rpc, whirlpool, param, lowerTickIndex, upperTickIndex, mintA, mintB, slippageToleranceBps = SLIPPAGE_TOLERANCE_BPS, funder = FUNDER) {
  ...
  const positionMint = await (0, import_kit14.generateKeyPairSigner)();
  ...
  instructions.push(
    (0, import_whirlpools_client6.getOpenPositionWithTokenExtensionsInstruction)({
      ...
      metadataUpdateAuth: (0, import_kit14.address)(
  ...
  return {
    ...
    initializationCost: (0, import_kit14.lamports)(nonRefundableRent)
  };
}
...
function createUninitializedTickArray(address7, startTickIndex, programAddress) {
  return {
    address: address7,
    data: {
      startTickIndex,
    ...
    space: 0n,
    executable: false,
    lamports: (0, import_kit15.lamports)(0n),
    programAddress
  };
}
...
async function swapInstructions(rpc, params, poolAddress, slippageToleranceBps = SLIPPAGE_TOLERANCE_BPS, signer = FUNDER) {
  ...
  swapInstruction.accounts.push(
    { address: tickArrays[3].address, role: import_kit15.AccountRole.WRITABLE },
    { address: tickArrays[4].address, role: import_kit15.AccountRole.WRITABLE }
  );
