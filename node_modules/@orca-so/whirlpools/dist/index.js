...
import { containsBytes as containsBytes2, getU8Encoder as getU8Encoder2, transformEncoder as transformEncoder2, getStructEncoder as getStructEncoder2, getU32Encoder as getU32Encoder2, getStructDecoder as getStructDecoder2, getU8Decoder as getU8Decoder2, getU32Decoder as getU32Decoder2, combineCodec as combineCodec2, getU64Encoder as getU64Encoder2, getU64Decoder as getU64Decoder2 } from "@solana/kit";
...
function getSetComputeUnitLimitInstructionDataEncoder() {
  return transformEncoder2(
    getStructEncoder2([
      ["discriminator", getU8Encoder2()],
      ["units", getU32Encoder2()]
    ]),
    (value) => ({
      ...value,
...
var DEFAULT_WHIRLPOOLS_CONFIG_ADDRESSES = {
  solanaMainnet: address3("2LecshUwdy9xi7meFgHtFJQNSKk4KdTrcpvaB56dP2NQ"),
  solanaDevnet: address3("FcrweFY1G9HJAHG5inkGB6pKg1HZ6x9UC2WioAfWrGkR"),
  eclipseMainnet: address3("FVG4oDbGv16hqTUbovjyGmtYikn6UBEnazz6RVDMEFwv"),
  eclipseTestnet: address3("FPydDjRdZu9sT7HVd6ANhfjh85KLq21Pefr5YWWMRPFp")
};
var DEFAULT_WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS = address3(
...
var DEFAULT_FUNDER = createNoopSigner2(DEFAULT_ADDRESS);
var FUNDER = DEFAULT_FUNDER;
function setDefaultFunder(funder) {
  if (typeof funder === "string") {
    FUNDER = createNoopSigner2(funder);
  } else {
    FUNDER = funder ?? createNoopSigner2(DEFAULT_ADDRESS);
  }
}
...
import { generateKeyPairSigner as generateKeyPairSigner2, lamports as lamports3 } from "@solana/kit";
...
import {
  ...
  getTransferSolInstruction as getTransferSolInstruction2
...
async function prepareTokenAccountsInstructions(rpc, owner, spec) {
  ...
  if (hasNativeMint && NATIVE_MINT_WRAPPING_STRATEGY === "seed") {
    ...
    const buffer = await crypto.subtle.digest(
      "SHA-256",
      Buffer.concat([
        Buffer.from(getAddressEncoder2().encode(owner.address)),
        Buffer.from(seed),
        Buffer.from(getAddressEncoder2().encode(TOKEN_PROGRAM_ADDRESS))
      ])
    );
    tokenAccountAddresses[NATIVE_MINT] = getAddressDecoder2().decode(
...
function orderMints(mint1, mint2) {
  const encoder = getAddressEncoder2();
...
async function wouldExceedTransactionSize(currentInstructions, instructionsToAdd) {
  const noopSginer = createNoopSigner3(
    address5("11111111111111111111111111111111")
  );
  const tx = await buildTransaction(
    [...currentInstructions, ...instructionsToAdd],
    noopSginer
  );
  const encodedTransaction = getBase64EncodedWireTransaction2(tx);
...
import { address as address6, generateKeyPairSigner as generateKeyPairSigner3, lamports as lamports4 } from "@solana/kit";
...
// src/swap.ts
import { AccountRole as AccountRole2, lamports as lamports5 } from "@solana/kit";
import {
  _TICK_ARRAY_SIZE,
...
async function swapInstructions(rpc, params, poolAddress, slippageToleranceBps = SLIPPAGE_TOLERANCE_BPS, signer = FUNDER) {
  ...
  swapInstruction.accounts.push(
    { address: tickArrays[3].address, role: AccountRole2.WRITABLE },
    { address: tickArrays[4].address, role: AccountRole2.WRITABLE }
  );
