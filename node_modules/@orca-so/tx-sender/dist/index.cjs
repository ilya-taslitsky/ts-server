...
function rpcFromUrl(url) {
  ...
  const transport = (0, import_web3.createDefaultRpcTransport)({ url });
  const rpc = createRpc({ api, transport });
  return rpc;
}
function normalizeAddresses(addresses) {
  return addresses?.map((addr) => (0, import_web3.address)(addr)) ?? [];
}
...
async function processJitoTipForTxMessage(message, signer, jito, chainId) {
  ...
  if (jitoTipLamports > 0) {
    return (0, import_web32.prependTransactionMessageInstruction)(
...
async function recentJitoTip(priorityFeePercentile) {
  ...
  return (0, import_web32.lamports)(BigInt(Math.floor(Number(data[key]) * 10 ** 9))).valueOf();
}
...
async function processComputeBudgetForTxMessage(message, computeUnits) {
  ...
  if (priorityFeeMicroLamports > 0) {
    message = (0, import_web33.prependTransactionMessageInstruction)(
  ...
  ixs.forEach((ix) => {
    if (ix.accounts) {
      ix.accounts.forEach((acc) => {
        if ((0, import_web33.isWritableRole)(acc.role)) writable.add(acc.address);
      });
    }
  });
...
async function getComputeUnitsForTxMessage(rpc, txMessage) {
  const estimator = (0, import_web34.getComputeUnitEstimateForTransactionMessageFactory)({
    rpc
  });
...
async function buildTransaction(instructions, feePayer, lookupTableAddresses) {
  return buildTransactionMessage(
    instructions,
    !("address" in feePayer) ? (0, import_web35.createNoopSigner)(feePayer) : feePayer,
...
async function buildTransactionMessage(instructions, signer, lookupTableAddresses) {
  ...
  if (lookupTableAddresses?.length) {
    ...
    const tables = lookupTableAccounts.reduce(
      (prev, account) => {
        if (account.exists) {
          (0, import_web35.assertAccountDecoded)(account);
    ...
    message = (0, import_web35.compressTransactionMessageUsingAddressLookupTables)(
      message,
      tables
    );
  }
  return (0, import_web35.signTransactionMessageWithSigners)(
...
async function prepareTransactionMessage(instructions, rpc, signer) {
  ...
  return (0, import_web35.pipe)(
    (0, import_web35.createTransactionMessage)({ version: 0 }),
    (tx) => (0, import_web35.setTransactionMessageLifetimeUsingBlockhash)(blockhash, tx),
    (tx) => (0, import_web35.setTransactionMessageFeePayerSigner)(signer, tx),
    (tx) => (0, import_web35.appendTransactionMessageInstructions)(instructions, tx)
  );
}
...
async function buildAndSendTransaction(instructions, payer, lookupTableAddresses, commitment = "confirmed") {
  ...
  (0, import_web36.assertTransactionIsFullySigned)(tx);
...
async function sendTransaction(transaction, commitment = "confirmed") {
  ...
  const encodedTransaction = (0, import_web36.getBase64EncodedWireTransaction)(transaction);
...
function getTxHash(transaction) {
  ...
  const txHash = (0, import_web36.getBase58Decoder)().decode(signature);
  return txHash;
}
